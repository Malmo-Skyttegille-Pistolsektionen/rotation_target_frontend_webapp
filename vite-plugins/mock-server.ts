...
// V1 SSE event types (kept local to mock-server.ts for legacy support)
const SSETypes = {
  ...
  TargetStatusChanged: 'target_status_changed',
...
export function mockServerPlugin(): Plugin[] {
  ...
  const audios = Object.entries(audiosData).map(([id, audio]: [string, any]) => ({
    id: Number(id),
    title: audio.title,
    readonly: true,
  }));
  ...
  lastThree.forEach(([id, audio]: [string, any]) => {
    audios.push({
      id: Number(id) + 1000,
      title: audio.title,
      readonly: false,
  ...
  // Simulation logic function to be called by the timer
  const runSimulationTick = () => {
    if (
      ...
      const totalTime = series.events.reduce((sum: number, event: any) => sum + event.duration, 0); // ms
  ...
  return [
    {
      ...
      configureServer(server: ViteDevServer) {
        // Start simulation timer only when dev server runs
        const simulationTimer = setInterval(runSimulationTick, 1000);
        ...
        server.middlewares.use((req, res, next) => {
          ...
          const unprotectedPaths = ['/admin-mode/enable', '/admin-mode/status'];
          if (
            isAdminModeEnabled() &&
          ...
          if (strippedPathname === '/targets/toggle' && req.method === 'POST') {
            ...
            res.writeHead(200);
            res.end(
              JSON.stringify({ message: `Target is now ${currentState.target_status_shown ? 'shown' : 'hidden'}` }),
            );
            return;
          }
          ...
          if (strippedPathname === '/programs' && req.method === 'GET') {
            ...
            const programList: ProgramSummary[] = Object.entries(programs).map(([id, data]: [string, any]) => ({
              id: Number(id),
              title: data.title,
          ...
          if (
            strippedPathname.startsWith('/programs/series/') &&
            strippedPathname.endsWith('/skip_to') &&
            req.method === 'POST'
          ) {
          ...
          if (
            strippedPathname.startsWith('/audios/') &&
            strippedPathname.endsWith('/delete') &&
            req.method === 'DELETE'
          ) {
          ...
          if (
            strippedPathname.startsWith('/programs/') &&
            strippedPathname.endsWith('/delete') &&
            req.method === 'DELETE'
          ) {
          ...
          if (
            strippedPathname.startsWith('/programs/') &&
            strippedPathname.endsWith('/update') &&
            req.method === 'PUT'
          ) {
        ...
        server.middlewares.use((req, res, next) => {
          ...
          if (url.pathname === SSE_PATHNAME) {
            ...
            const heartbeat = setInterval(() => {
              ...
              console.log(
                `[SSE] Heartbeat #id=${heartbeatId} sent to client ${clientIp}:${clientPort} (${clients.length} total)`,
              );
              heartbeatId++;
            }, HEARTBEAT_INTERVAL);
